...
export default function UpdatePassword(){
 ...
 return <div className="auth-page single"><div className="auth-panel full"><div className="auth-top"><Link to="/" className="brand"><img src="/elvra-logo.png" alt="ELVRA"/><span>ELVRA</span></Link></div><div className="auth-content narrow"><div className="eyebrow">ELVRA / PASSWORD</div><h1>{t.changePassword}</h1><p>Choose a strong password you will remember.</p><form className="form-stack" onSubmit={submit}><label>{t.newPassword}<input type="password" minLength="6" required value={p} onChange={e=>setP(e.target.value)} /></label><label>{t.confirmPassword}<input type="password" minLength="6" required value={c} onChange={e=>setC(e.target.value)} /></label><button className="primary-btn" disabled={busy}>{busy?'...':t.updatePassword}<CheckCircle2 size={18}/></button></form>{msg&&<div className="success-box">{msg}</div>}{err&&<div className="error-box">{err}</div>}</div></div></div>
}
